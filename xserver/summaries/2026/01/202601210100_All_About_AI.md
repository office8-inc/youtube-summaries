# 📺 2026年にノートパソコンでローカルAIを実行（AMD Ryzen AI PRO 128GB）
...
AMD Ryzen AI Pro MPU（128GB RAM）搭載ノートパソコンで、完全オフラインのローカルAI環境を構築する方法を紹介する動画です。飛行機内など、インターネット接続がない環境でも、Olama、GPT-OSS、Quen Coderなどの最新AIモデルを使って、コーディング、画像認識（OCR）、Agentic Workflowが実行できることを実証しています。40-51トークン/秒の高速処理が可能で、クラウドにデータを送信しないため完全なプライバシーとセキュリティが確保されます。開発者やAI活用に興味がある方向けの実践的な内容です。

## ⭐ 重要なポイント

- **高速なローカル処理**: GPT-OSS 20Bで約40トークン/秒、Quen Coder 30Bで51トークン/秒を達成、読むスピードを超える速度
- **多様なAIモデル**: 会話用（GPT-OSS）、コーディング用（Quen Coder）、画像認識用（Quen 3 VL）の3種類を組み合わせて使用可能
- **完全なプライバシー**: 全ての処理がローカルで完結し、データがクラウドに送信されないため、機密コードや個人情報も安全に扱える
- **簡単なセットアップ**: Olamaを使えば、コマンド一つでモデルのインストールと実行が可能、GUIとCLI両方で利用可能
- **Agentic Workflowの実現**: Open Coderと組み合わせることで、オフライン環境でもファイル生成やコード編集などの自動タスク実行が可能
