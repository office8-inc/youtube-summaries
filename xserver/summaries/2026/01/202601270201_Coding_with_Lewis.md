...
この動画は、AIに「身体」と「目」を与え、自然の中で自律的に探索させる壮大なハードウェアプロジェクトの記録です。Raspberry Pi 5、カメラモジュール、RCカーを組み合わせ、Claude Opus 4.5を搭載した完全自律型ロボットカーを構築します。ハードウェア初心者の製作者が、数ヶ月にわたる試行錯誤の末、AIが独自に判断して森の中を探索できるシステムを完成させました。AIエージェント、コンピュータビジョン、ハードウェア統合に興味がある方、特にClaude Codeを使った実践的なプロジェクトに挑戦したい方に最適な内容です。

## ⭐ 重要なポイント

- **完全自律型AIロボット**: Raspberry Pi 5とClaude Opus 4.5を組み合わせ、カメラ映像を元に独自判断で進路を決定する自律型ロボットカーを実現
- **ハードウェア構成**: 1,100ドルのRCカー、16メガピクセルカメラ、サーボドライバーボード、4G通信モジュールを統合したシステム
- **MCPサーバーで制御**: Claude Codeに独自ツール（左右旋回、前進など）を提供し、画像プロンプトから適切な行動を判断させる仕組み
- **Journey Grid技術**: 動画を送れない制約を、6枚の連続画像をコラージュにすることで解決し、AIに「移動中の出来事」を理解させる
- **深度推定の活用**: Apple Depth Proで障害物までの距離を推定し、衝突回避や後退判断を可能にする高度な視覚認識機能を実装
