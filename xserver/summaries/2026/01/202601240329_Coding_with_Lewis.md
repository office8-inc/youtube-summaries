...
2017年2月28日、Amazon Web Servicesのエンジニアが実行したコマンドのタイプミスが、1億5000万ドル以上の損失を引き起こした事件を紹介する動画です。S3ビリングサブシステムのサーバーを削除するつもりが、意図した以上のサーバーを停止させてしまい、世界中の数百万の企業が利用するS3が完全にダウンしました。Slack、Docker、Expediaなど多数のサービスが影響を受け、復旧まで3時間以上かかりました。小さなミスが大きな影響を及ぼす教訓的な事例です。
