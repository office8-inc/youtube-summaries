# 📺 ADHDの机をロボットアームで自動片付け — 20分の掃除を避けるために40時間
...
この動画は、ADHDで散らかりがちな机を片付けるために、XARM 7というロボットアームで机上の物体を自動仕分けするシステムを作った記録です。本人いわく「20分の掃除を避けるために40時間の作業」という、典型的なオーバーエンジニアリングです。  
仕組みは、アームを最上部まで上げて机全体を見下ろし、Florence 2で物体が何かを検出し、SAM 2.1でセグメントし、深度画像で各マスクをロボット座標系の3D位置へ持ち上げる、という流れ。処理のほとんどはノートPCのNvidia GPU上でローカル実行しています。  
コーディングはスポンサーのDevonに任せ、Cerebras上で秒間1,000トークンで動くSWE 1.7のlightningモードを使用。ビジョンが絡むとフィードバックループが弱点になるため、エージェントが停止するたびに深度カメラで撮影させるstop hookを仕込んでいます。  
途中でアーケードボタンを壊しかけたり、ガムテープが掴めなかったりと失敗も出ますが、掴める隙間の幅を測る方式に改良し、最後はIKEAのペグボードに3Dプリントしたパーツを掛けて、ビンごと棚に片付けるところまで到達しました。

## ⭐ 重要なポイント

- 使っているのはXARM 7。7軸で約8〜10ポンドを持ち上げられ、0.1mmの精度で動く。速度よりも精度が強みで、それが棚のビンを抜き差しする用途に効いている。  
- 視覚パイプラインはFlorence 2（物体検出）→ SAM 2.1（セグメンテーション）→ 深度画像で3D位置へ、の3段構成。カメラはIntel RealSenseで、通常のカメラ2つに赤外線を加えて深度値を取る。印刷したグリッドでのキャリブレーションは基本1回だけで済む。  
- コーディングエージェントはDevonのSWE 1.7（lightningモード、Cerebrasで秒間1,000トークン）。LLMはビジョンが得意ではないため、エージェントが停止するたびに深度カメラで撮影させるstop hookを追加し、実機の状態を見せながら進めた。  
- 初期版は掴み方で失敗した。ガムテープは人間なら開口部を探して掴むが、アームは横から掴もうとして失敗する。そこでセグメントした縁の最も近い2点間の距離を測り、開口幅に応じて掴む位置を決めるよう改良した。  
- 片付け先はIKEAのペグボード。3Dプリントの「カセット」と呼ぶパーツを自作して掛けられるようにし、アームが棚のビンを出し入れできる構成にした。「AIエージェントが証明したのは、遅くても最終的に正しくやってくれればいいということ」——遅いアームでも離席中に片付いていればいい、という設計思想。
