# 📺 GPT-6でKalshiのAI取引ボットをゼロから作る（完全ガイド）
...
この動画は、OpenAI がリリースした GPT-6 Astra と Codex を使って、予測市場 Kalshi のニューヨーク市の最高気温マーケットを狙う自動取引ボットをゼロから組み上げる実況ガイドです。  
予測の根拠には Google DeepMind の天気予報モデル WeatherNext 3 を BigQuery と Earth Engine 経由で使い、アクセス申請は「最大7日」と案内されたものの実際は1〜2日で通ったと述べています。  
約40分かかったバックテストの結果、開場から1分後のタイミングが最も強く、スコアは64対74（低いほど良い）で約13%の改善だったため、そこを狙って建てる方針を決めています。  
最後は AWS のオハイオリージョンに T3 micro インスタンスを立てて24時間稼働させる構成まで進め、VPS費用と手数料も勝敗計算に入れるよう釘を刺しています。

## ⭐ 重要なポイント

- 構成は **GPT-6 Astra（high reasoning）+ Codex**。Codex がなければ Claude Code でも同じことができると補足している。  
- データ源は Google DeepMind の **WeatherNext 3** と Earth Engine。Google Cloud で BigQuery API と Earth Engine API を有効化し、OAuth 2.0 クライアントIDを作って gcloud CLI でログインする手順を実演。  
- **決済に使う観測所の確認が重要**。Kalshi のこのイベントはマンハッタンのセントラルパーク観測所（ベルヴェデーレ城のすぐ南の岩山）で決まる。JFK とは数度ずれる可能性があるため、市場ルールごと Codex に読ませて確認している。  
- バックテストは約40分。開場+1分、+2分、+5分、+10分、+15分、+30分を比較し、**開場+1分が64対74で約13%良い**という結果から、決定時刻を開場1分後に設定。ただし「エッジを確立するにはデータが足りない」とモデル自身が留保している。  
- 前日の手動テストでは80〜81度のバケットを56セントで買い、動画収録時点で63セント（約12%のプラス）で推移。マイアミ市場ではニューヨークほど強い結果が出なかったため、この動画はNYを選んだと明かしている。  
- 本番は Kalshi の API キーと秘密鍵を .env とテキストファイルに置き、AWS オハイオの T3 micro（安さ優先）で24時間稼働。口座残高は100ドル。**VPSの運用費と取引手数料を計算に含めること**を注意点として挙げている。
