...
LightrixがリリースしたLTX-2は、完全オープンソースのAI動画生成モデルで、単なるウェブラッパーではなく、モデルウェイト、トレーニングコード、ベンチマークまで完全公開されています。Nvidia RTX GPUに最適化されており、家庭用ハードウェアでも4K解像度・音声付きの高品質な動画を生成可能です。ComfyUIと統合され、テキストから動画、画像から動画、動画から動画への変換、さらにカメラ制御用のLoRAまで提供されており、開発者やスタジオが自由にカスタマイズできる真のオープンソースプロジェクトです。

## ⭐ 重要なポイント

- **史上最も重要なAI動画リリース**: 完全オープンウェイト、トレーニングコード完全公開により、真の意味でのオープンソースを実現（単なるデモやラッパーではない）
- **家庭用ハードウェアで実用的**: RTX 4090（24GB VRAM）で快適に動作し、蒸留版（distilled）・量子化版を使えばより低スペックでも利用可能
- **4K・音声対応のマルチモーダル**: 最大4K解像度、ネイティブ音声生成、テキスト→動画、画像→動画、動画→動画など複数のパイプラインをサポート
- **ComfyUIで簡単に利用可能**: ノードベースのビジュアルインターフェースで、ワークフロー全体を可視化しながら操作。カメラ制御LoRA（ドリー移動など）も提供
- **完全版と蒸留版の使い分け**: 完全版（2分27秒）は最高品質、蒸留版（53秒）は高速イテレーション用。目的に応じて使い分けることで効率的な制作が可能
