...
本動画では、Googleがリリースした最新モデル「Gemini 3.1 Pro」の性能を、ARC-AGI 2・BrowseComp・Apex Agents・Terminal Bench・TAO 2などの新世代エージェントベンチマークで詳しく解説しています。Gemini 3.1 ProはARC-AGI 2で31%から77%へと3か月で劇的に向上し、ターミナル操作やウェブ調査などのエージェント能力で最高スコアを記録しています。AIの進化はQAベンチマークの時代から「実際の業務を自律的にこなせるか」を測るエージェント時代へと完全に移行しており、その急速な進歩を理解したい人に向けた内容です。

## ⭐ 重要なポイント

- Gemini 3.1 ProはARC-AGI 2（抽象推論テスト）で前バージョンの31.1%から77%へとわずか3か月で大幅向上し、AIの推論能力が急速に進化していることを示した
- BrowseComp（ウェブ調査エージェントベンチマーク）でGemini 3.1 Proは85.9%を記録し、Claude Opus 4.6（84%）やGPT-5.2を上回り世界トップとなった
- Apex Agentsベンチマーク（コンサル・投資銀行・法律などのホワイトカラー業務）では33.5%に達しており、完全自動化（100%）にはまだ距離があるが急速に伸びている
- Terminal Bench 2.0ではGemini 3.1 Proが68.5%でトップに立ち、CLIを使った技術的タスクにおいてAIが人間を支援できる水準に近づいている
- TAO 2ベンチマーク（対話型カスタマーサービスエージェント）ではClaude Opus 4.6が91.9%でリードしており、人との協調的なコミュニケーション能力はまだ差がある
