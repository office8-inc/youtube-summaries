# 📺 AIで組んだ「執行アルファ」の仕組みがPolymarketで効いている
...
この動画は、Polymarketで「予測そのもの」ではなく「注文の執行」を最適化してきた話です。  
Codex・Claude Code・OpenCodeといったAIツールを使い、ここ数週間かけて、注文をどれだけ速く出せるか、板のキューでどの位置を取れるかを研究してきたと語ります。  
Polymarketの板はFIFO（先入先出）なので、キューの先頭にいれば先に約定します。「2077年末までの米国景気後退」市場に1セントで5株を出したところ、自分の前にいる推定株数は12、後ろは110万株という位置が取れたと実例を見せています。  
予測の精度ではなく機械的・技術的なエッジを狙う話で、WebSocketの常時接続、パラメータや認証情報の事前準備、サーバーの設置場所、HTTP接続の再利用といった具体的な改善点を挙げています。

## ⭐ 重要なポイント

- 狙っているのは予測の当たり外れではなく「execution alpha（執行アルファ）」。注文をどれだけ速く出し、板のキューで良い位置を取れるかという技術的な優位のこと。  
- 例として10,000%の利益という数字を挙げ、Bitcoinの5分アップダウン市場で5セントで5株を買い、決済時に5ドルが戻ってきたケースを見せている。毎回起きるわけではなく、良いキュー位置があってはじめて約定確率が上がる。  
- 「2077年末までの米国景気後退」という先の長い市場に1セントで5株を出した例では、自分の前にいる推定株数が12、後ろが110万株。板が急変して1セントまで下がれば、先に約定する側に回れる。  
- PolymarketはFIFO（先入先出）方式なので、たとえば100人しか約定しない場面でも、キューの先頭にいる自分は1セントで約定し、後ろの110万株は約定しない。  
- 具体的な改善策は、ポーリングをやめてWebSocketの常時接続にする、パラメータと認証情報を事前に用意しておく、VPSなどサーバーの設置場所を選ぶ、HTTPの持続接続を再利用する、プロファイル署名と実行レイテンシを詰める、の5点。今後はBitcoin・暗号系の反復的な市場に注力し、KalshiやHyperliquidでも試すという。
