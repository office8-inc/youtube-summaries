# 📺 全てのデバイスに入っているのに誰も知らないコード：SQLiteの知られざる誕生秘話
...
世界で最も広く使われているソフトウェアでありながら、ほとんどの人がその存在を知らないSQLiteの誕生秘話を描いたドキュメンタリー調の動画です。2000年に軍の誘導ミサイル駆逐艦向けシステム開発で外部DBサーバーの障害に悩んだエンジニアRichard Hippが、「ファイル1つで完結するDB」を自力で開発し、オープンソース公開したことで世界中に普及した経緯を紹介しています。エンジニアリングの歴史や、偉大な技術が生まれる背景に興味がある方に最適な内容です。

## ⭐ 重要なポイント

- **普及規模**: 現在、地球上に1兆個以上のSQLiteデータベースが稼働中。iPhone・Android・Chrome・Safari・Firefox・Windows・Macのほぼ全デバイスに搭載されている
- **誕生のきっかけ**: 軍の艦艇ダメージコントロールシステム開発中にInformixサーバーの頻繁なダウンに悩んだRichard Hippが「サーバー不要のDBがあれば」と着想
- **革新的な設計思想**: サーバープロセス不要で、データを単一ファイルとして直接読み書きする。外部依存がなく、アプリが起動できる環境なら必ずDBも機能する
- **普及の転機**: オープンソース公開後、わずか2〜8MBのRAMしか持たないPalmPilotへの移植に成功したことでコミュニティが爆発的に注目
- **知名度ゼロの偉大さ**: 一般ユーザーにはほぼ無名だが、現代のデジタルインフラを支える最重要ソフトウェアの一つ
