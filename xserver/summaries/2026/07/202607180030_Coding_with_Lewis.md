# 📺 ADHD対策で2台のロボットを作ったら、連携し始めてしまった話
...
この動画は、ADHDによる先延ばし対策として、投稿ペナルティ用スクリプトと物理的にPCを閉じるロボットを組み合わせた実験を紹介しています。
通知を無視できる従来アプリと違い、「行動に強制力を持たせる仕組み」を作ることで集中を維持しようとする試みが主題です。
開発者向けには、検知ロジック、Webhook連携、ESP32制御、監視基盤（Sentry）まで含めた実装面の学びがあります。
最終的には、生産性向上と心理的負荷のトレードオフをどう設計するかが重要だというメッセージに着地しています。

## ⭐ 重要なポイント

- 先延ばし検知は「フォーカス時間中に90秒以上アイドル」や「許可外ドメインを約4分継続閲覧」などの条件でトリガーされる。
- 違反時には確認なしでAPI投稿する設計にして、行動コストを明確化している。
- 物理デバイスはESP32とサーボ／ソレノイド機構で構築し、25分ごとに強制的に休憩へ移行させる。
- 誤検知の被害が大きいため、Sentryでスタックトレースやイベント履歴を監視し、実害前に修正できる体制を作っている。
- 自動化は効果がある一方、恐怖ベースの運用は負担も大きく、仕組み設計には安全性と持続可能性が必要だと示している。
