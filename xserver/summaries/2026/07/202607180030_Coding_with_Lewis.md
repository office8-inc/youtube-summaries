# 📺 ADHDを直すためにロボットを2台作ったら、2台が勝手に連携し始めた
...
ADHDを抱える投稿者が、あらゆる生産性アプリに挫折した末に「お願いするのではなく強制する」2台の仕掛けを自作する動画です。  
1台は評判を人質に取るソフトで、集中時間中にサボると事前に書いた「懺悔文」を本物のSNSアカウントへ自動投稿します。もう1台は物理的にノートPCを閉じるロボットです。  
検知スクリプトはアクティブなウィンドウや放置時間を監視し、許可リスト外のサイトに約4分連続・90秒放置などの閾値を超えると確認なしで即投稿。誤検知対策としてスポンサーのSentryでバグ（許可リスト照合の大文字小文字ミス）を根本原因まで追跡した経緯も紹介されます。  
ロボットはポモドーロの25分ごとに、12Vソレノイドとバネで腕を振り下ろしPCを「ギロチンのように」閉じます。2台を連携させた初日、ビデオ通話中に強制切断・投稿・PC閉鎖が発生し、「システムは何も間違っていない＝自分が本当にサボっていた」ことがかえって怖かったと振り返り、自罰的な生産性のあり方を問いかけます。

## ⭐ 重要なポイント

- ADHD対策として、通知を無視できる従来の生産性アプリを見限り「許可を求めず強制する」2台の仕掛けを自作。1台は評判を、もう1台はノートPC本体を狙う。  
- ソフト側は、集中ブロック中にサボると事前に用意した過激な「懺悔文」を本物のSNSアカウントへ確認ダイアログなしで自動投稿する。閾値は許可リスト外ドメインに約4分連続、または90秒の放置など。  
- スポンサーのSentryで実際に誤投稿（misfire）を検知。原因は「アクティブアプリと許可リストを大文字小文字を無視せず照合していた」ことで、ドキュメントのタブがリスト外扱いになっていた。Sentryが根本原因まで追跡し修正を提示した。  
- 物理ロボットはポモドーロ式の25分タイマー。低速なサーボ版は指1本で止められたため、12Vソレノイドとバネで腕を高速に振り下ろす「産業用ギロチン」風の本命版を製作。ESP32でWi-Fi接続し、webhookで外部から起動できる。  
- 2台を連携させた初日、ビデオ通話中に強制的に切断・投稿・PC閉鎖が起きた。「システムは正しく動作した＝自分が実際にサボっていた」点がかえって怖く、自罰的な生産性向上のあり方に疑問を投げかけている。
