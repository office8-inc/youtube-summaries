# 📺 「プロンプトよりLOOPSを作れ」—swyxが語るAI開発の本質
...
この動画は、swyxへのインタビューを通して、AI開発が「単発プロンプト」から「継続的に回るループ設計」へ移行している現状を解説しています。  
大規模トークン消費が当たり前になった開発現場の実態と、ROIを意識しない運用が失敗につながるリスクの両面が語られます。  
最先端チームがどれほどの規模でAIを実運用しているか、具体的な数字を交えて共有される点も見どころです。  
AIプロダクトを作るエンジニアや起業家にとって、スケール戦略とコスト管理を同時に考える重要性を学べる内容です。

## ⭐ 重要なポイント

- 1秒あたり1万〜数十万トークン規模のデモが登場しており、AI開発の処理スケールは急速に拡大している。  
- イベント参加者7,000人中、想定50人に対して約300人が「トークンビリオネア」だったという事例が、高い普及速度を示している。  
- 先端チームでは1日あたり数十億トークン級の利用報告もあり、実運用フェーズが一気に進んでいる。  
- ただし「大量消費＝価値」ではなく、実ユーザー価値につながらない実装はコストと時間を浪費すると警告。  
- 結論として、重要なのはプロンプトの巧拙より、改善を回し続けるループ（設計・計測・改善）を作ることだと強調される。
