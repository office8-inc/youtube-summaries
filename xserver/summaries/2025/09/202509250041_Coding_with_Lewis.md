...
この動画は、Slackが2020年のCOVIDパンデミック時にどのようにして急激なトラフィック増加を乗り越えたかを解説しています。SlackはもともとシンプルなLAMPアプリとして2013年に誕生し、シャード型MySQLデータベースで成長を支えてきました。しかしCOVID直撃時、わずか1週間でデータベースクエリが50%以上急増するという前例のない事態が発生しました。Slackがシステム崩壊を防げた理由は、3年前から進めていたVitess（VTest）への移行にありました。エンジニアリングチームの先見性と、オープンソースコミュニティへの貢献が、危機的状況を乗り越える鍵となった事例です。

## ⭐ 重要なポイント

- **SlackはLAMPスタック＋MySQLシャードから始まった**: 2013年のリリース当初はシンプルな構成だったが、1,000万人超のユーザーを抱えるにつれてスケーリングの限界に直面した
- **Vitess（VTest）への移行が命綱になった**: YouTubeが開発しオープンソース化したVitessを採用し、3年かけてMySQLのプロキシ層を整備していたことが、COVID危機の際に即座に対応できた理由
- **COVID発生時、1週間でクエリ数が50%以上増加**: 毎秒150万クエリが突如225万クエリ以上になるという前代未聞のスパイクが発生し、事前の準備なしには対応不可能だった
- **Vitessはライブシャード分割を可能にした**: 数百万人のユーザーが利用している最中に、過負荷状態のデータベースをダウンタイムなしで水平分割できた点が決定的な差だった
- **オープンソースの力と協力文化が大規模システムを支える**: VitessはYouTubeからSlackへ、さらにコミュニティ全体へと広がった。エンジニアの透明性ある情報公開がソフトウェア業界全体を強くする
