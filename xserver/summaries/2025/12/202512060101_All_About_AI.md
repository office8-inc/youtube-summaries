# 📺 Shipmas Day 1: 自律型AIソーシャルメディア動画変換アプリ
...
この動画では、Claude Code Opus 4.5を使って、横長動画を縦型動画に自動変換するアプリを一から構築します。YOLO（You Only Look Once）で顔の座標を検出し、Whisperで音声を文字起こし、AIが興味深いクリップを選択、FFmpegで編集・結合するまでを完全自動化しました。10分の動画から30〜40秒のショート動画を生成し、顔を追跡しながらスムーズにクロップする技術を実演。AI開発者や動画クリエイター向けの実践的なプロジェクトです。

## ⭐ 重要なポイント

- **YOLO顔検出技術**: You Only Look Onceライブラリで動画内の顔の座標をリアルタイム追跡。話している人物や口の動きまで検出可能
- **インテリジェントなクロップ**: 検出した顔の座標をもとに、FFmpegで16:9動画を9:16縦型フォーマットに変換。顔が常に中央に配置されるようスムーズに追従
- **自動クリップ選択**: Whisperで全文を文字起こし後、Claude Opusが10分動画から面白い部分を自動選択し、30秒のショート動画を生成
- **フリッカー問題の解決**: 初期バージョンでは顔の間で画面が行き来していたが、信頼度の高い話者に固定することで滑らかな映像を実現
- **わずか30分で完成**: プランニングから実装、テスト、改善まで含めて30分以内で実用的なアプリを構築。Claude Code Opus 4.5の強力な開発支援を実証
